...
export default function ClientHeader({ onMenuClick }: ClientHeaderProps) {
  ...
  return (
    <header className="h-16 bg-[#FCFBFC] border-b border-slate-200 flex items-center justify-between px-4 md:px-8 z-10 sticky top-0">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-base md:text-lg font-semibold text-slate-800 truncate max-w-[120px] sm:max-w-none">{pageTitle}</h2>
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div className="relative hidden sm:block">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="pl-10 pr-4 py-2 bg-slate-100 border-none rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#8D1A42]/20 w-48 md:w-56"
          />
        </div>
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            <div className="text-left hidden sm:block">
              <p className="text-sm font-semibold text-slate-800 leading-tight max-w-[100px] truncate">{user?.name ?? 'Client'}</p>
            </div>
          ...
          {/* Dropdown Menu */}
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-lg border border-slate-100 py-1.5 z-50">
              {/* User info */}
              <div className="px-4 py-3 border-b border-slate-100">
                <p className="text-sm font-semibold text-slate-800 truncate">{user?.name ?? 'Client'}</p>
