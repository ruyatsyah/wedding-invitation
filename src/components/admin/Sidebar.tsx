...
export default function Sidebar({ isOpen = false, onClose, isMinimized = false, onToggleMinimize }: SidebarProps) {
  ...
  const sidebarContent = (isMobile: boolean) => (
    <div className="flex flex-col justify-between min-h-full" style={{ background: 'linear-gradient(180deg, #fefefe 0%, #faf7f8 100%)' }}>
      {/* Brand */}
      <div>
        <div className={`px-6 pt-7 pb-6 ${!isMobile && isMinimized ? 'px-2' : ''}`}>
          <div className={`flex items-center ${!isMobile && isMinimized ? 'justify-center' : 'justify-between'}`}>
            <div className="flex items-center gap-2">
              ...
              {(!isMinimized || isMobile) && (
                <div>
                  <h2 className="text-base font-extrabold tracking-tight whitespace-nowrap" style={{ color: '#8D1A42' }}>
                    Kabar Bahagia
                  </h2>
          ...
          {menuItems.map((item, idx) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center ${(!isMobile && isMinimized) ? 'justify-center px-0' : 'gap-3 px-4'} py-2.5 rounded-xl text-[13px] font-medium transition-all duration-200`}
                title={(!isMobile && isMinimized) ? item.name : undefined}
                style={
                  isActive
                    ? {
                        background: '#8D1A42',
              ...
              >
                {item.icon}
                {(!isMinimized || isMobile) && <span className="whitespace-nowrap">{item.name}</span>}
              </Link>
            );
          })}
        </nav>
      ...
      <div className={`px-4 pb-5 ${!isMobile && isMinimized ? 'px-2' : ''}`}>
        <div className="border-t border-slate-100 pt-4 space-y-1">
          {/* Profile dropdown trigger */}
          <div className="relative" ref={profileRef}>
            <button
              onClick={() => setProfileOpen(!profileOpen)}
              className={`w-full flex items-center ${(!isMobile && isMinimized) ? 'justify-center px-0' : 'gap-3 px-2'} py-2 rounded-xl hover:bg-slate-50 transition-colors`}
              title={(!isMobile && isMinimized) ? (adminUser?.name ?? 'Admin') : undefined}
            >
            ...
            {/* Dropdown */}
            {profileOpen && (
              <div className={`absolute bottom-full mb-1 bg-white rounded-xl shadow-lg border border-slate-100 py-1.5 z-50 ${!isMobile && isMinimized ? 'left-12 w-48' : 'left-0 right-0'}`}>
                <button
  ...
  return (
    <>
      ...
      <aside className={`hidden md:block shrink-0 h-screen fixed top-0 left-0 bottom-0 z-40 overflow-y-visible border-r border-slate-100 transition-all duration-300 ${isMinimized ? 'w-20' : 'w-60'}`}>
